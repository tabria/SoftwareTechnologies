...
public class FilmController {
	...
	public String createProcess(Model model, FilmBindingModel filmBindingModel) {
		...
		if (filmBindingModel.getName().equals("") || filmBindingModel.getGenre().equals("") ||
				filmBindingModel.getDirector().equals("") || filmBindingModel.getYear() < 1890){
			return "redirect:/";
		}
	...
	public String editProcess(Model model, @PathVariable int id, FilmBindingModel filmBindingModel) {
		...
		Film filmDb = filmRepository.findOne(id);
		if (filmDb == null) {
			return "redirect:/";
		}
		filmDb.setName(filmBindingModel.getName());
		filmDb.setGenre(filmBindingModel.getGenre());
		filmDb.setDirector(filmBindingModel.getDirector());
		filmDb.setYear(filmBindingModel.getYear());
		filmRepository.saveAndFlush(filmDb);
		return "redirect:/";
