...
public class CountWorkingDays {
    public static void main(String[] args) {
        ...
        Date dateEnd = null;
        try {
            dateStart = formatter.parse(scanner.nextLine());
    ...
    private static boolean isBusinessDay(Date date, List<Holiday> holidays) {
        ...
        boolean arror = holidays.contains(new Holiday(date));
        Holiday hol = new Holiday(date);
        for(int i=0; i<holidays.size(); i++){
            if (holidays.get(i).day == hol.day && holidays.get(i).month == hol.month){
                return false;
            }
        }
